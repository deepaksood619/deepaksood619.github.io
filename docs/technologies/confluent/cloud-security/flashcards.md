...
I | SSO provides centralized policy management for MFA, password enforcement, and user termination | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | Common SAML providers include Okta, Active Directory, and Google accounts | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | Scales better than managing individual credentials per application | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | API keys are simpler; OAuth provides better scalability and centralized management | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | Resource-specific keys follow principle of least privilege for production use | Source: 21-authenticating-users-and-applications.md#cloud-api-keys-vs-resource-specific-api-keys |
...
I | **Why:** Continuity and security. **How to apply:** Create service accounts for all production apps | Source: 21-authenticating-users-and-applications.md#api-keys-vs-service-accounts |
...
I | **Why:** Prevents service disruption. **How to apply:** Never delete old key before verifying new key works | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | **Why:** Scales to thousands of identities without hitting API key limits. **How to apply:** Group clients with similar access needs | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | OAuth is industry standard; most companies already use OAuth for other cloud resources | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | **How to apply:** Audit legacy clients before enabling OAuth; they may need upgrades or continue using API keys | Source: 21-authenticating-users-and-applications.md#authentication |
...
I | Simplifies multi-service authentication but requires proper RBAC/ACL permissions to authorize operations | Source: 21-authenticating-users-and-applications.md#global-api-keys |
...
I | **Why:** Separation of authentication (the key) from authorization (RBAC/ACLs). **How to apply:** Always assign proper permissions before using global keys | Source: 21-authenticating-users-and-applications.md#global-api-keys |
...
I | **How to apply:** Use resource-scoped keys when accessing only one resource type | Source: 21-authenticating-users-and-applications.md#global-api-keys |
...
I | **Why:** Reduces credential juggling. **How to apply:** For single-resource access, still prefer resource-specific keys | Source: 21-authenticating-users-and-applications.md#use-a-global-api-key-when |
...
I | Permissions are tied to individual identities, not roles | Source: 22-authorizing-using-rbac-and-acls.md#acls |
...
I | **Why:** Labor-intensive verification for compliance. **How to apply:** Use ACLs only for small identity counts; prefer RBAC for large orgs | Source: 22-authorizing-using-rbac-and-acls.md#acls |
...
I | **How to apply:** Use RBAC for environment/org-level access control | Source: 22-authorizing-using-rbac-and-acls.md#acls |
...
I | **How to apply:** Test ACL changes before assuming they're active | Source: 22-authorizing-using-rbac-and-acls.md#acls |
...
I | **Why:** RBAC scales better and simplifies compliance verification. **How to apply:** Prefer RBAC for organizations with hundreds/thousands of identities | Source: 22-authorizing-using-rbac-and-acls.md#rbacs |
...
I | **Why:** Safer and simpler to verify roles than individual ACL identities. **How to apply:** Use RBAC as the default authorization method | Source: 22-authorizing-using-rbac-and-acls.md#rbacs |

^learnkit-110358362
T | OrganizationAdmin Role Requirement |
Q | Which RBAC role is required to create service accounts? |
A | OrganizationAdmin role. |
I | Other admin roles (EnvironmentAdmin, CloudClusterAdmin, ResourceOwner) can grant/revoke permissions on controlled resources but not create new service accounts | Source: 22-authorizing-using-rbac-and-acls.md#rbac-considerations |
...
I | **How to apply:** Basic clusters don't support granular RBAC; use ACLs or upgrade cluster type | Source: 22-authorizing-using-rbac-and-acls.md#rbac-considerations |
...
I | **How to apply:** Start with least privilege; grant DeveloperRead for read-only access to topic data | Source: 22-authorizing-using-rbac-and-acls.md#rbac-considerations |
...
I | **Why:** Provides default user permissions. **How to apply:** Customize these defaults based on organizational needs | Source: 22-authorizing-using-rbac-and-acls.md#rbac-considerations |
...
I | Automates role assignment based on external group membership | Source: 22-authorizing-using-rbac-and-acls.md#group-mapping |
...
I | **How to apply:** Start with basic for simple use cases; use advanced when complex filtering is needed | Source: 22-authorizing-using-rbac-and-acls.md#types-of-group-mappings |
...
I | **Why:** API keys are static credentials outside the SSO flow. **How to apply:** Manually assign permissions to user API keys separately | Source: 22-authorizing-using-rbac-and-acls.md#limitations |
...
I | **Why:** Prevents new users from being locked out. **How to apply:** Combine with least-privilege specific group mappings | Source: 22-authorizing-using-rbac-and-acls.md#best-practices-for-group-mappings |
...
I | **Why:** DENY rules are evaluated first and override all ALLOW rules. **How to apply:** Use DENY sparingly; it overrides everything | Source: 22-authorizing-using-rbac-and-acls.md#acls-and-rbac-order-of-precedence |
...
I | **Why:** ACL ALLOW and RBAC roles are additive. **How to apply:** Either mechanism can grant access independently | Source: 22-authorizing-using-rbac-and-acls.md#acls-and-rbac-order-of-precedence |
...
I | For KRaft clusters, use StandardAuthorizer (`org.apache.kafka.metadata.authorizer.StandardAuthorizer`) | Source: 22-authorizing-using-rbac-and-acls.md#authorizer |
...
I | **Why:** Separates broker-to-controller auth from client-to-broker auth. **How to apply:** Ensure both broker and client principals have proper permissions | Source: 22-authorizing-using-rbac-and-acls.md#kraft-principal-forwarding |
...
I | **How to apply:** Extract exact subject name from certificates when creating ACLs | Source: 22-authorizing-using-rbac-and-acls.md#principal |
...
I | **How to apply:** Use Kerberos principal names when creating ACLs for Kerberos-authenticated clients | Source: 22-authorizing-using-rbac-and-acls.md#principal |
...
I | **Why:** Useful for public topics but violates least privilege. **How to apply:** Use sparingly and document justification | Source: 22-authorizing-using-rbac-and-acls.md#wildcard-principals |
...
I | Each operation maps to specific Kafka API calls | Source: 22-authorizing-using-rbac-and-acls.md#operations |
...
I | **Why:** Prevents zombie producers. **How to apply:** Each transactional producer needs unique transactional.id and ACL permission | Source: 22-authorizing-using-rbac-and-acls.md#transactional-id |
...
I | DENY takes precedence over ALLOW | Source: 22-authorizing-using-rbac-and-acls.md#permissions |
...
I | **How to apply:** Use PREFIXED for topic naming conventions like `orders.*` | Source: 22-authorizing-using-rbac-and-acls.md#use-prefixed-acls-pattern-types |

^learnkit-531881114
T | Super Users Default Behavior |
Q | By default, who can access resources with no associated ACLs? |
A | Only super users. |
I | **How to apply:** Override with `allow.everyone.if.no.acl.found=true` in server.properties if needed (not recommended) | Source: 22-authorizing-using-rbac-and-acls.md#super-users |
...
I | Confluent Cloud encrypts both by default: TLS 1.2 for motion, encryption for rest | Source: 23-understanding-encryption.md#encryption |
...
I | **How to apply:** BYOK and private networking are optional enhancements beyond defaults | Source: 23-understanding-encryption.md#encryption |
...
I | **Why:** Renders encrypted data unreadable without key access. **How to apply:** Required for regulated industries | Source: 23-understanding-encryption.md#byok |
...
I | **Why:** BYOK is not end-to-end encryption. **How to apply:** Use end-to-end encryption if you need to prevent all provider access | Source: 23-understanding-encryption.md#byok |
...
I | **Why:** Key management shifts from fully managed to customer-managed. **How to apply:** Monitor key access permissions continuously | Source: 23-understanding-encryption.md#byok |
...
I | **Why:** Encryption is only reversible with the key. **How to apply:** Implement robust key backup and disaster recovery procedures | Source: 23-understanding-encryption.md#byok |
...
I | **How to apply:** If you have an existing cluster, create new cluster with BYOK and use Replicator to migrate data | Source: 23-understanding-encryption.md#byok |
...
I | **How to apply:** Configure automatic rotation in AWS KMS, Azure Key Vault, or Google Cloud KMS | Source: 23-understanding-encryption.md#byok |
...
I | Comprehensive cluster-wide encryption | Source: 23-understanding-encryption.md#byok-details |
...
I | Small cost: ~3 cents per 10,000 KMS API calls on AWS/GCP invoice | Source: 23-understanding-encryption.md#byok-details |
...
I | **How to apply:** Verify key type when configuring KMS | Source: 23-understanding-encryption.md#byok-details |
...
I | **How to apply:** Use CSFLE for selective field encryption; CSPE for full payload encryption | Source: 23-understanding-encryption.md#csfle-vs-cspe |

^learnkit-939212569
T | CSFLE Rule Definition |
Q | How does CSFLE identify which fields to encrypt? |
A | By defining fields via tags in the schema. |
I | Uses ENCRYPT rule type (Domain Rule) applied to tagged fields | Source: 23-understanding-encryption.md#csfle-vs-cspe |
...
I | **Why:** CSPE doesn't require schema tagging. **How to apply:** Use as stepping stone to CSFLE for complex schemas | Source: 23-understanding-encryption.md#encrypt-data-at-rest-using-self-managed-encryption-keys |
...
I | **Why:** CSFLE enables fine-grained access control at field level. **How to apply:** Use CSFLE when different consumers need different field access | Source: 23-understanding-encryption.md#csfle-vs-cspe |
...
I | Stream of authorization attempts with user, sequence, and timestamp | Source: 24-auditing-and-monitoring.md#audit-logs |
...
I | **Why:** Forensic record for security breaches and compliance proof. **How to apply:** Monitor logs continuously for anomalies | Source: 24-auditing-and-monitoring.md#benefits-of-audit-logs |
...
I | **How to apply:** Replicate to another cluster or external system for longer retention | Source: 24-auditing-and-monitoring.md#audit-logs |
...
I | **Why:** Prevents tampering with audit trail. **How to apply:** Request dedicated audit log API key for read-only access | Source: 24-auditing-and-monitoring.md#audit-logs |
...
I | Industry standard rapidly becoming universal for event data | Source: 24-auditing-and-monitoring.md#audit-logs |
...
I | Each includes unique ID, event source, event context, and event data | Source: 24-auditing-and-monitoring.md#audit-log-types |
...
I | **Why:** Keeps audit logs lightweight and focused on authorization tracking. **How to apply:** Don't rely on audit logs for data recovery | Source: 24-auditing-and-monitoring.md#audit-log-events |
...
I | **Why:** SIEM integration enables automated security monitoring. **How to apply:** Configure replication during cluster setup | Source: 24-auditing-and-monitoring.md#auditing-and-monitoring |
...
I | Service Organization Control compliance for service organizations | Source: 25-maintaining-compliance-and-privacy.md#compliance-and-privacy |
...
I | **How to apply:** Required for any organization handling credit card data | Source: 25-maintaining-compliance-and-privacy.md#compliance-and-privacy |
...
I | Global standard for information security | Source: 25-maintaining-compliance-and-privacy.md#compliance-and-privacy |

^learnkit-299024269
T | GDPR and CCPA |
Q | What are GDPR and CCPA? |
A | GDPR is the General Data Protection Regulation (EU). CCPA is the California Consumer Privacy Act. |
I | **How to apply:** Both regulate personal data handling; GDPR is EU-wide, CCPA is California-specific | Source: 25-maintaining-compliance-and-privacy.md#compliance-and-privacy |

^learnkit-935260605
T | HIPAA Compliance |
Q | What is HIPAA? |
A | The Health Insurance Portability and Accountability Act - regulates healthcare data privacy and security. |
I | **How to apply:** Required for healthcare organizations handling patient data | Source: 25-maintaining-compliance-and-privacy.md#compliance-and-privacy |
...
I | **Why:** Enhanced security by restricting access to trusted networks. **How to apply:** All requests from non-included IPs are denied | Source: ip-filtering.md#ip-filtering |
...
I | **How to apply:** Create IP groups first, then create IP filters that use those groups | Source: ip-filtering.md#key-components |
...
I | **How to apply:** Use Organization scope for company-wide policies; Environment scope for team-specific policies | Source: ip-filtering.md#key-components |
...
I | **How to apply:** Assign operation groups based on which types of operations need IP restrictions | Source: ip-filtering.md#key-components |
...
I | **Why:** IP filtering focuses on management operations, not data plane. **How to apply:** Use private networking for Kafka data protection | Source: ip-filtering.md#limitations |
...
I | **Why:** Validates both allow and deny rules. **How to apply:** Test from both inside and outside trusted networks | Source: ip-filtering.md#example |
