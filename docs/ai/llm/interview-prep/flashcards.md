...
I | Example: "artificial" → "art", "ific", "ial". Enables handling of diverse languages and rare words.
...
I | Used to estimate API costs and context window usage
...
I | Larger windows (e.g., 32K tokens) improve coherence but increase computational costs
...
I | Example: "cryptocurrency" → "crypto" + "currency"
...
I | Quadratic complexity in self-attention: O(n²) where n is sequence length
...
I | Most computationally intensive phase; base model has no task-specific behaviors yet
...
I | Faster and cheaper than pretraining; adds task-specific behaviors
...
I | QLoRA can fine-tune 70B parameter models on single GPU
...
I | Used in BERT; enables understanding of semantic relationships
...
I | Improves coherence in dialogue and document tasks; without NSP, BERT performs worse on all metrics
...
I | Unlike autoregressive models that only see prior tokens
...
I | NSP teaches longer-term dependencies across sentences
...
I | Every new capability needs new data + retraining cycle
...
I | Encoder processes input, decoder generates output
...
I | Enables the model to focus on relevant tokens dynamically
...
I | Example: "Bark is cute and he is a dog" - different heads link 'dog' to name, gender, description
...
I | Without it, "I am good" and "good am I" would be identical
...
I | Scaled dot product measures relevance, softmax normalizes to probabilities
...
I | Transformers solve long-term dependency problem that plagues RNNs
...
I | CNN approach becomes impractically large for long dependencies
...
I | Unlike RNNs which suffer from exponential decay over long sequences
...
I | More coherent than greedy decoding which only picks single best token
...
I | Temperature=0 is deterministic, temperature=1 uses raw probabilities
...
I | Top-p adapts to context - narrow distribution when confident, wide when uncertain
...
I | Task determines optimal randomness level
...
I | Beam search explores multiple hypotheses to find better overall coherence
...
I | Pretraining corrupts sentences (masking) and reconstructs them
...
I | Pretraining predicts next word in sequence (autoregressive)
...
I | Shapes strengths: generation vs comprehension
...
I | Example: "Je vous en prie" → "You are welcome" not "I you in pray"
...
I | Example: "dog" embedding evolves during training to reflect pet-related contexts
...
I | Both are subword tokenization reaching desired vocabulary size
...
I | Combined to distinguish sentences and preserve word order
...
I | Encoding captures inherent position relationships mathematically
...
I | Steps: index, retrieve, rank, generate using retrieved context
...
I | Recursive for PDFs with tables, semantic for long reports
...
I | Best for general enterprise search covering both paraphrased questions and exact terms
...
I | Solves context-fragmentation issue of small chunks (200 tokens) lacking surrounding information
...
I | LLMs process info at beginning/end better than middle of prompt
...
I | Ensures answer is grounded only in retrieved documents, not hallucinated
...
I | Tests how well answer addresses the query
...
I | Faster than using an LLM for classification
...
I | RRF: result high in either list stays at top of final result
...
I | ReAct for dynamic tasks, Plan-and-Execute for complex tasks with parallelizable steps
...
I | Example: validator node routes back to retriever on error
...
I | Can't fit hundreds of tool descriptions in a single prompt
...
I | Prevents user from staring at 30s loading spinner
...
I | Agents can adapt and self-correct dynamically
...
I | Clear delegation model for complex multi-step tasks
...
I | Prevents agents from losing context of what others did
...
I | Reliability critical for production agent systems
...
I | Improves reasoning and decision-making over single-shot generation
...
I | Enables customization without retraining entire model
...
I | Reduces computational load while maintaining performance
...
I | Mimics human reasoning for complex multi-step tasks
...
I | Example: "Classify this review as positive/negative" without sentiment training
...
I | Ideal for niche tasks like specialized classification
...
I | Reduces cost of handling 50K+ token vocabularies
...
I | Valuable for question answering and entity recognition
...
I | Techniques like LoRA maintain performance across domains
...
I | Must be addressed for ethical and effective use
...
I | Extent to which model goals align with human values
...
I | Improves fairness and accuracy systematically
...
I | Only way at the moment - no persistent external memory mechanism
...
I | Typical LLM: 7B-175B parameters vs traditional ML: millions
...
I | Lower perplexity = better training. 10-20% reduction is noteworthy.
...
I | Intrinsic: model-centric. Extrinsic: task-centric (e.g., speech recognition accuracy).
...
I | Expands use cases to visual QA and complex dialogues
...
I | More stable and scalable multimodal training
...
I | Widely used in LLM architectures for robust training
...
I | Formula: DKL(P||Q) = Σ P(x)log(P(x)/Q(x))
...
I | Demonstrates lack of consistent relational understanding
...
I | Ensures robust generalization to unseen data
...
I | Generative: creation. Discriminative: classification.
