---
slug: /databases-sql/aws-redshift/readme
title: Redshift
description: Discover Amazon Redshift's architecture, ETL processes, workload management, pricing, and best practices for optimized data warehousing and query performance.
created: 2025-06-10
last_update: 2026-06-12
---
- [Amazon Redshift](databases-sql/aws-redshift/intro.md)
- [Architecture](databases-sql/aws-redshift/architecture.md)
- [Documentation](databases-sql/aws-redshift/documentation.md)
- [ETL in Redshift](databases-sql/aws-redshift/etl-redshift.md)
- [Workload management (WLM) and query monitoring rules (QMR)](databases-sql/aws-redshift/wlm-qmr.md)
- [Deep dive / Best Practices](databases-sql/aws-redshift/deep-dive-best-practices.md)
- [Pricing / Sizing](databases-sql/aws-redshift/pricing-sizing.md)
- [Redshift SQL Queries / Commands](databases-sql/aws-redshift/redshift-sql-queries-commands.md)
- [Others](databases-sql/aws-redshift/others.md)
- [redshift-to-snowflake-migration](data-warehouses/redshift-to-snowflake-migration.md)
	- [Tableau Migration](ai/data-visualization/tableau/migration.md)
